...
int MP1Node::introduceSelfToGroup(Address *joinaddr) {
    ...
    if ( 0 == memcmp((char *)&(memberNode->addr.addr), (char *)&(joinaddr->addr), sizeof(memberNode->addr.addr))) {
        ...
        msg = new MessageHdr();
        msg->msgType = JOINREQ;
        msg->sharedList = memberNode->memberList;
...
void MP1Node::handlePing(MessageHdr* msg) {
    ...
    for(int i = 0; i < msg->sharedList.size(); i++){
        
        MemberListEntry* node = inMemberList(msg->sharedList[i].id, msg->sharedList[i].port);  
        if(node == nullptr){
            MemberListEntry* memberEntry = &msg->sharedList[i];
            ...
            if(msg->sharedList[i].heartbeat > node->heartbeat){
                node->heartbeat = msg->sharedList[i].getheartbeat();
