...
void MP1Node::sendMessage(Address* sendAddress, MsgTypes msg) {
    MessageHdr* msg = new MessageHdr();
    msg->msgType = t;
    // msg->member_vector = memberNode->memberList;
    msg->addr = &memberNode->addr;
    emulNet->ENsend(&memberNode->addr, sendAddress, (char*)msg, sizeof(MessageHdr));
}   
...
void MP1Node::nodeLoopOps() {
    ...
    for (std::vector<MemberListEntry>::iterator it = memberNode->memberList.begin();
        it != memberNode->memberList.end(); ++it)
    {
        if(it->shouldDeleteMember) {
            memberNode->memberList.erase(it);
        } else if(par->getcurrtime() >= TREMOVE + it->timestamp) {
            logAddress(it->id, it->port, false);
            //memberNode->memberList.erase(it);
            it->shouldDeleteMember = true;
        }
    }

    for (int i = 0; i < memberNode->memberList.size(); i++) {
        Address* address = getMemberAddress(memberNode->memberList[i].id, memberNode->memberList[i].port);
